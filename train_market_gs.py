...
import time
import os
from model import ft_net, ft_net_dense, PCB_gs
...
parser.add_argument('--name', default='ft_ResNet50_pcb_market_e_gs', type=str, help='output model name')
parser.add_argument('--data_dir', default='./dataset/market_rename', type=str, help='training dir path')
        ...
        for phase in ['train', 'val']:
            ...
            for data in dataloaders[phase]:
                ...
                if not opt.PCB:
                    ...
                    weight=sig(part[0])*sig(part[1])*sig(part[2])*sig(part[3])*sig(part[4])*sig(part[5])
                    _, preds = torch.max(score.data, 1)

                    loss = criterion(part[0], labels)
                    for i in range(num_part - 1):
                        loss += criterion(part[i + 1], labels)
                    p_gb = outputs[-1] * weight
                    loss_gb = criterion(p_gb, labels)
                    loss = loss + loss_gb
...
if not opt.PCB:
    ...
    ignored_params += (list(map(id, model.classifier0.parameters()))
                       ...
                       + list(map(id, model.classifier_gb.parameters()))
    ...
    optimizer_ft = optim.SGD([
        ...
        {'params': model.classifier_gb.parameters(), 'lr': 0.1},
